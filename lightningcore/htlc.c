...
{
    union printable_types locktime;
    union printable_types sha256;
    locktime.abs_locktime = &h->expiry;
    sha256.sha256 = &h->rhash;
	...
	return tal_fmt(ctx, "{ id=%"PRIu64
		       ...
		       " src=%s }",
		       h->id, h->msatoshi,
		       type_to_string_(ctx, stringify(struct abs_locktime), locktime),
		       type_to_string_(ctx, stringify(struct sha256), sha256),
